...
func makeVariableTerm (tok *Token )(*Term){
	r := new(Term)
	r.Typ = TermVariable
...
func makeNumberTerm(tok *Token )(*Term){
	r := new(Term)
	r.Typ = TermNumber
	r.Literal = tok.literal
	return r
...
func makeAtomTerm(tok *Token )(*Term){
	r := new(Term)
	r.Typ = TermAtom
	r.Literal = tok.literal
	return r
	...
	if err == nil{
		if tok.typ == tokenVariable{
			if nextTok,_ := lexer.next(); nextTok.typ == tokenComma{
				return nil,true,false,makeVariableTerm(tok)
			}else if nextTok.typ == tokenRpar{
				return nil, false,true,makeVariableTerm(tok)
			}else {
				return ErrParser, false,false,nil
			}
		}else if tok.typ == tokenNumber{
			if nextTok,_ := lexer.next(); nextTok.typ == tokenComma{
				return nil,true,false,makeNumberTerm(tok)
			}else if nextTok.typ == tokenRpar{
				return nil, false,true,makeNumberTerm(tok)
			}else {
				return ErrParser, false,false,nil
			}
			...
			if nextTok,_ := lexer.next(); nextTok.typ == tokenLpar{
				...
				return nil,true,false,makeAtomTerm(tok)
			}else if nextTok.typ == tokenRpar{
				return nil,false,true,makeAtomTerm(tok)
			}
